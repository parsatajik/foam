  ...
  return (
    <footer>
      <div className="max-w-6xl mx-auto py-16 sm:px-10 px-5 pb-0">
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-4 sm:grid-cols-2 mt-4 mb-8">
              ...
              <ul>
                {section.links.map((link, linkIndex) => (
                  <li key={linkIndex} className="my-2">
                    <Link
                      href={link.href}
                      className="group inline-flex cursor-pointer items-center justify-start gap-1 text-muted-foreground duration-200 hover:text-foreground hover:opacity-90 text-sm"
                    >
                      {link.icon && link.icon}
                      {link.text}
